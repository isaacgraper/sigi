...
| OQ-04 | RN07 requires an "área de competência" that exists on no entity. What is it — órgão, setor, categoria de insumo? | SPEC-0001, RN07 | **Answered by the data:** the operation scopes by **unidade** and by **grupo de materiais** — compradores own grupos, gestores de unidade own units. Both entities now exist in the model (OQ-26), so RN07 becomes implementable as a two-axis scope on `USUARIO`. *(2026-09-10)* The adoption lands in **SPEC-0003**, not SPEC-0001: RN07 restricts which *insumos* a servidor may see, and neither reference table is built yet, so a criterion in the auth spec would assert nothing observable. | **Resolved — spec change pending in SPEC-0003** |
...
| OQ-09 | Will the entity register with Gov.br for OAuth, and by when? | SPEC-0001, RF01 | **Wrong question, and now settled.** Nobody mentioned Gov.br in the 17/08 meeting; servidores already hold credentials from the entity's TI and the discussion was about **Entra ID**. Gov.br is citizen-to-state federation and does not fit an internal-servidor audience. **ADR-0010** decides it: institutional OIDC as the primary mechanism, local credentials as a switchable contingency, Gov.br **cut** rather than deferred. Implemented in SPEC-0001 v0.3 (AC-0001-19..22, -24). Still needs the TI to confirm Entra ID and hand over tenant, client and redirect URI — but nothing blocks on that. | **Assumed** |
| OQ-10 | Why does the profile screen collect CPF? | SPEC-0001, LGPD | **Do not collect**, and now enforced rather than recommended. No requirement needs it and it raises the LGPD burden for nothing. AC-0001-14 used to anonymise a `cpf` field that exists in no entity; SPEC-0001 v0.3 removed the reference. Under art. 6, III the cheapest compliance measure is not holding the data. | **Assumed** |
...
| OQ-30 | Where does a single-use invitation or reset token travel? | **Resolved (2026-09-20, SPEC-0001 v0.8).** In the request body. A path lands in access logs, proxies and browser history, and the cost of moving it rises once a client binds to it. The link handed to the gestor still carries it as a query parameter, because a link has to be openable. |
| OQ-31 | Restoring AC-0001-32's guarantee that a gestor cannot take over a member's account | **Open, needs a mail transport.** SPEC-0001 v1.0 hands the reset link to the triggering gestor, because there is no channel to the member. A gestor can therefore reset a credential and use the link. The audit row and the session revocation make that detectable, not impossible. When SMTP exists, send the link to the member's own address and delete the link from the trigger response; nothing else has to change. Until then this is a trust assumption resting on the audit trail. |
| OQ-32 | Which proxy, if any, may set the client address the throttle counts against | **Open, needs the deployment topology.** AC-0001-33 counts on `request.client.host` and ignores `X-Forwarded-For`, because trusting that header unconditionally lets anyone bypass the throttle by setting it. On-premise behind a reverse proxy, every request will appear to come from the proxy and share one ceiling. The fix is a trusted-proxy list plus reading the header only from those hops; it needs to know what sits in front of the application, which the entity has not said. |
...
- Bring the four highest-impact items to the next stakeholder meeting; the rest
  ...
  **OQ-28** (two definitions of `RF05`/`RN11` in `docs/` — decide before any
  ...
  of any meeting.
- *(2026-09-10)* **OQ-09** and **OQ-10** became `Assumed` under ADR-0010 and are
  no longer questions, but OQ-09 leaves one concrete ask for the entity's TI:
  confirm Entra ID and hand over the tenant ID, client ID and redirect URI. It
  blocks the first real login, not the build.
