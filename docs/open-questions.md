...
| OQ-30 | Invitations are delivered by the gestor, who copies the activation link and passes it on. Should SIGI also send it by e-mail, and if so with which SMTP relay, sender address and retention? | SPEC-0001 AC-0001-10 | Not a blocker: AC-0001-10 v0.6 makes the gestor the delivery channel, which is what an entity with locked-down mail does anyway. E-mail would be a **second** channel behind the same link, and needs the entity's TI to supply the relay. Worth asking at the same time as the Entra ID details (OQ-09). | **Open** |
