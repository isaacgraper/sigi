...
| **AC-0001-29's loser gets a deadlock, not the specified error.** Measured over 40 races: exactly one deactivation always succeeds, and the other returns `40P01` 39 times out of 40 against `SI005` once. The trigger's `FOR UPDATE` runs after the UPDATE already locked its own target, so the two transactions lock in opposite order | Already measured, so this is a requirement on step 10 rather than a risk: the service takes `pg_advisory_xact_lock` before the update, **and** maps `40P01` to 409 `ULTIMO_GESTOR` as a second line. Without both, the endpoint returns 500 on a foreseeable race |
...
| ~~**No Docker locally.**~~ **Retired 2026-09-10.** PostgreSQL 16 binaries turned out to be installed on the development machine, so the harness takes `TEST_DATABASE_URL_ADMIN` and falls back to `testcontainers`. The DB-backed tests run in both places | Already done: the substrate's 30 tests were written and run against a real PostgreSQL 16.13 rather than deferred to CI |
